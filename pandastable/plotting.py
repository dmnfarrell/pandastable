...
class PlotViewer(Frame):
    ...
    def plot2D(self, redraw=True):
        ...
        bw = kwds['bw']
        # print (kwds)
        if not self._checkNumeric(data) and kind != 'venn':
        ...
        if by != '':
            ...
            if kwargs['subplots']:
                ...
                if 'sharey' in kwargs and kwargs['sharey']:
                    self.autoscale()
                if 'sharex' in kwargs and kwargs['sharex']:
                    self.autoscale('x')
        ...
        '''if table:
            ...
            if self.table.child is not None:
    ...
    def _doplot(self, data, ax, kind, subplots, errorbars, useindex, bw, yerr, kwargs):
        ...
        styles = []
        if bw and kind not in ['pie', 'heatmap']:
            cmap = None
            kwargs['color'] = 'k'
    ...
    def scatter(self, df, ax, alpha=0.8, marker='o', color=None, **kwds):
        ...
        for i in range(s, plots):
            ...
            if kwds['subplots']:
                ax.set_title(cols[i])
            if colormap is not None and kwds['colorbar']:
        ...
        if kwds['legend'] == 1 and not kwds['subplots']:
            ax.legend(cols[1:])
    ...
    def scatter3D(self, data, ax, kwds):
        ...
        def doscatter(data, ax, color=None, pointlabels=None):
            ...
            for i in range(2, l):
                z = X[:, i]
                color_ = color or cmap(float(i) / l)
                h = ax.scatter(x, y, z, edgecolor='black', c=color_, linewidth=lw,
...
class MPLBaseOptions(TkOptions):
    ...
    def __init__(self, parent=None):
        ...
        opts = self.opts = {'font': {'type': 'combobox', 'default': self.defaultfont, 'items': fonts},
                            'fontsize': {'type': 'scale', 'default': 12, 'range': (5, 40), 'interval': 1,
                                         'label': 'font size'},
                            'marker': {'type': 'combobox', 'default': '', 'items': markers},
                            'linestyle': {'type': 'combobox', 'default': '-', 'items': linestyles},
                            'ms': {'type': 'scale', 'default': 5, 'range': (1, 80), 'interval': 1,
                                   'label': 'marker size'},
                            ...
                            'cscale': {'type': 'combobox', 'items': scales, 'label': 'color scale',
                                       'default': 'linear'},
                            ...
                            'linewidth': {'type': 'scale', 'default': 1.5, 'range': (0, 10), 'interval': 0.1,
                                          'label': 'line width'},
                            'alpha': {'type': 'scale', 'default': 0.9, 'range': (0, 1), 'interval': 0.1,
                                      'label': 'alpha'},
                            ...
                            'pointsizes': {'type': 'combobox', 'items': datacols, 'label': 'point sizes',
                                           'default': ''},
                            }
        self.kwds = {}
        return
...
class AnimateOptions(TkOptions):
    ...
    def __init__(self, parent=None):
        ...
        self.groups = grps = {'data window': ['increment', 'window', 'startrow', 'delay'],
                              'display': ['expand', 'usexrange', 'useyrange', 'tableupdate', 'smoothing',
                                          'columntitle'],
        ...
        opts = self.opts = {'increment': {'type': 'entry', 'default': 2},
                            ...
                            'columntitle': {'type': 'combobox', 'default': '', 'items': datacols,
                                            'label': 'column data as title'},
